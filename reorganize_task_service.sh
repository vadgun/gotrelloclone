...
echo "📦 Reorganizando estructura del task-service..."
...
if command -v git &> /dev/null; then
  ...
  git mv $SERVICE_DIR/config/task_config.go $INFRA_DIR/config/config.go
  ...
  mv $SERVICE_DIR/config/task_config.go $INFRA_DIR/config/config.go
...
echo "✅ task-service reorganizado con la carpeta infra 🚀"
